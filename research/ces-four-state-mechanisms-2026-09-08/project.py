...
def make(baseline,selected,private):
    ...
    for r in read(baseline,fields):
        ...
        if p0=='missing' or p1=='missing':raise ValueError('四态完整队列存在真正空缺，必须另定目标')
        ...
        uid=hashlib.sha256(('LH264-fold-v1:'+k).encode()).hexdigest()
        rec={'key':uid,'fold':int(uid[:16],16)%5,'s':s,'y0':('D','I','R','not_sure').index(p0),'y':('D','I','R','not_sure').index(p1) if s else None,
             'housing':0 if own==1 else 1 if own==2 else 2,'latino_employed':int(l),'age':agecat,'education':edu,'groups':groups_from_flags(l,rr)}
        masked.append(rec);truth.append({'key':uid,'y':('D','I','R','not_sure').index(p1)})
    ...
    assert len(masked)==11009 and len(ids)==11009 and len(sel)==6175
